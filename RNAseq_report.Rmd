...
```{r, echo=FALSE}
dr = "/.../" #Full path to salmon quantification
  ##Mus_musculus
  ##Drosophila_melanogaster
  ##Homo_sapiens
organism <- "Homo_sapiens"
gtf_file_path <- "..." # Gencode annotations
...
txdb <- makeTxDbFromGFF(gtf_file_path,
...
dup_row<-which(duplicated(sig_genes_names$ensembl_id))
if (length(dup_row) != 0) {
    sig_genes_names<-sig_genes_names[-which(duplicated(sig_genes_names$ensembl_id)),]
}
...
for (go in 1:length(topGO_BP$GO.ID)){
  ...
  go_gene_list$ensembl_id <-paste0('<a href="',"http://www.ensembl.org/",organism,"/Gene/Summary?db=core;g=",go_gene_list$ensembl_id,'">',go_gene_list$ensembl_id,"</a>")
